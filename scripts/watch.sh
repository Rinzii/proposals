...
WATCH_SPEC="${1:-cpp/annotation_customization.bs}"
...
if command -v watchexec >/dev/null 2>&1; then
    watchexec -w cpp -w scripts -w Makefile -- "${MAKE_CMD[@]}"
elif command -v entr >/dev/null 2>&1; then
    find cpp scripts -type f \( -name '*.bs' -o -name '*.sh' \) | sort | entr -c "${MAKE_CMD[@]}"
else
